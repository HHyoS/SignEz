...
// 면적이 가장 큰 contour를 찾습니다.
    ...
    private fun approxPolyDP(contour: MatOfPoint, epsilon: Double, closed: Boolean): MatOfPoint2f {
    ...
    private suspend fun detectPhoto() = coroutineScope {
        launch {
            ...
            if (originalImage != null) {
                ...
                val moduleHeight: Float =
                    height.toFloat() / (signage.heightCabinetNumber * cabinet.moduleColCount)
                Log.d("Signage Size", "$width,  $height")
                Log.d("Module Size", "$moduleWidth,  $moduleHeight")

                val resultId = analysisViewModel.saveResult(signage.id)
                Log.d("ImageProcess", "resultId = $resultId")
                ...
                try {
                    ...
                    Log.d("ImageProcess", "points = $points")
                    ...
                    Log.d("ImageProcess", "errorModuleList = $errorModuleList")
                    for (errorModule in errorModuleList) {
    ...
    private suspend fun getPredictions(
        ...
        Log.d("Detection", "$imgScaleX,  $imgScaleY")
        // 640x640 resize
        val resizedMat = Mat()
    ...
    private fun outputFloatsToErrorModules(
        ...
        for (i in 0 until mOutputRow) {
            if (outputs[i * mOutputColumn + 4] > scoreThreshold) {
                ...
                if (mutableMap[moduleX * 10000 + moduleY] == null || mutableMap[moduleX * 10000 + moduleY]!! < score.toDouble()) {
                    mutableMap[moduleX * 10000 + moduleY] = score.toDouble()
                }
    ...
    private fun sortPoints(points: MutableList<Point>): MutableList<Point> {
        val sortedPoints = ArrayList<Point>()
